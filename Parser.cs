...
{
    [Guid("8C034F6A-1D3F-1211-BC99-B73873D8C297")]
    [ProgId("GoldParserForNet.Parser")]
    [ComVisible(true)]
    ...
    public class Parser
    {
        public readonly string Version = "5.08";
        ...
        {
            return ((Reduction)CurrentReduction).ParentRule.RuleNonterminal.Name;
        }

        public bool IsCurrentTokenComment()
        {
            if (CurrentToken.Type == SymbolType.Noise && CurrentToken.Name != "whitespace" && CurrentToken.Name != "NewLine")
            { return true; }
            else
            { return false; }
        }
        ...
        {
            return _currentPosition.Line + 1;
        }
        public int CurrentColumnNumber()
        {
            return _currentPosition.Column + 1;
        }
        ...
        /// <returns></returns>
        public bool OpenTextString(string text)
        {
        ...
        {
            return _expectedSymbols.Count;
        }

        public Symbol Tokens(int index)
        {
            return _expectedSymbols[index];
        }
        ...
        /// <returns></returns>
        public bool LoadCompiledGrammar(string path)
        {
            using (FileStream fileStream = File.Open(path, FileMode.Open, FileAccess.Read))
            {
                using (var binaryReader = new BinaryReader(fileStream))
                {
                    return LoadCompiledGrammar(binaryReader);
                }
            }
        }
        ...
        public ParseMessage Parse()
        {
            ...
            while (!done)
            {
                ...
                if (_inputTokens.Count == 0)
                {
                    ...
                    if (read.Type == SymbolType.Noise && read.Group != null && String.Compare(read.Name, "NewLine", StringComparison.InvariantCultureIgnoreCase) != 0)
                    {
                        if (_firstNoise == null)
                        {
                            _firstNoise = read;
                        ...
                        if (_lastToken != null)
                        {
                            _lastToken.EndNoise = read;
                        }
                    }
                    else if (read.Type == SymbolType.Terminal)
                    {
                        _lastToken = read;
                        read.BeginNoise = _firstNoise;
                        _firstNoise = null;
                    }
                }
                else
